...
epochStepValues = np.zeros(epocsToDo)
...
@jit(nopython=True)
def generateANN(rowUValues, rowDValues, epochTeRMSEValues, epochVaRMSEValues, epocsToDo, epochStepValues,df_tr, df_va,df_te, weights, biases, hiddenNodes, inputs, momentum, endStepSize):
    oldMSE = 0
    originalMomentum = momentum
    for curEpoch in range(1, epocsToDo+1, 1):
        #annealing learning paramater
        stepSize = endStepSize +((startStepSize-endStepSize)*(1-(1/(1+(math.e)**(10-((20 * curEpoch)/epocsToDo))))))
        epochStepValues[curEpoch-1] = stepSize
        oldWeights = np.empty_like(weights)
        oldWeights[:] = weights
        oldBiases = np.empty_like(biases)
        oldBiases[:] = biases
        #calculate weight decay
        batchValues = np.zeros(len(weights)+len(biases))
        omega = 0
        ...
        for row in range(0, len(df_tr), 1):
            rowBatchWeights = np.zeros(len(weights))
            rowBatchBiases = np.zeros(len(biases))
            ...
            #add delta and to node values to their column sums for hidden nodes
            for node in range(0, hiddenNodes, 1):
                currentInput = 0
                for toNode in range(node*inputs, ((node*inputs)+inputs), 1):
                    rowBatchWeights[toNode] =rowBatchWeights[toNode] + rowDValues[node] * df_tr[row, currentInput]
                    currentInput = currentInput + 1
                rowBatchBiases[node] = rowBatchBiases[node] + (rowDValues[node]) * 1
            #add delta and to node values to their column sums for output node
            currentInput = 0
            for toNode in range((inputs*hiddenNodes), (inputs*hiddenNodes) + hiddenNodes, 1):
                rowBatchWeights[toNode] = rowBatchWeights[toNode] + (outputD) * rowUValues[currentInput]
                currentInput = currentInput + 1
            rowBatchBiases[hiddenNodes] = rowBatchBiases[hiddenNodes] + outputD * 1
        #update weights and biases using the batch values
        for updateWeight in range(0, len(weights), 1):
            weights[updateWeight] = weights[updateWeight] + (stepSize * (rowBatchWeights[updateWeight] / len(df_tr)))
        for updateBias in range(0, len(biases), 1):
            biases[updateBias] = biases[updateBias] + (stepSize * (rowBatchBiases[updateBias] / len(df_tr)))
        ...
        if oldMSE == 0:
            ...
            if (oldMSE * 1.01) < mse:
                ...
                biases[:] = oldBiases
                #print("here 1")
            elif ((oldMSE * 1.04) > mse) and (oldMSE < mse):
        ...
        print (testRMSE)
        #print (weights)
generateANN(rowUValues, rowDValues, epochTeRMSEValues, epochVaRMSEValues,epocsToDo, epochStepValues,df_tr, df_va,df_te, weights, biases, hiddenNodes, inputs, momentum, endStepSize)
#print (biases)
#print (weights)
